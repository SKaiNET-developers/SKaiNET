...
/**
 * SIMD-vectorized Q5_K matmul on the JDK Vector API.
 *
 * Identical pipeline to [PanamaVectorQ4KMatmulKernel], with one extra step:
 * each code is 5-bit, so the low nibble (from `qs`) is OR'd with the 5th bit
 * pulled from the `qh` high-bit plane. For super-block group `j` (0..3), the
 * low sub-block uses `qh` bit `2j` and the high sub-block uses bit `2j + 1`,
 * with `qh` indexed by the intra-group position (same `idx` as `qs`). See
 * [sk.ainet.lang.tensor.data.Q5_KTensorData] / `DequantOps.dequantQ5KFromBytes`.
 *
 * Numerical equivalence with the scalar reference is within FMA +
 * reordered-reduction tolerance; verified via parity tests.
 */
public object PanamaVectorQ5_KMatmulKernel : Q5KMatmulKernel {
        ...
        parallelChunks(outputDim, schedule) { startO, endO ->
            ...
            for (o in startO until endO) {
                ...
                for (blockIdx in 0 until blocksPerInputDim) {
                    ...
                    // Sub-scale decode via ggml `get_scale_min_k4` (identical to Q4_K).
                    ...
                    // The 5th bit comes from qh[idx], bit (2*groupJ) for lo, (2*groupJ+1) for hi.
                    val qhOffset = blockBase + QH_OFFSET
                    val qsOffset = blockBase + QS_OFFSET
                    val inputBlockBase = inputOffset + blockIdx * BLOCK_SIZE
                    for (groupJ in 0 until 4) {
                        val qsRegion = qsOffset + groupJ * 32
                        val sbLo = 2 * groupJ
                        val sbHi = sbLo + 1
                        val bitLo = (2 * groupJ).toByte()
                        val bitHi = (2 * groupJ + 1).toByte()
                        ...
                        // SIMD body — single qs + qh load feeds both nibble vectors.
                        while (idx + floatStep <= SUB_BLOCK_SIZE &&
                            ...
                            val qhVec = ByteVector.fromArray(byteSpeciesForFloat, weight, qhOffset + idx)
                            val loNib = byteVec.and(0x0F.toByte())
                            val hiNib = byteVec.lanewise(VectorOperators.LSHR, 4.toByte())
                            val fifthLo = qhVec.lanewise(VectorOperators.LSHR, bitLo)
                                .and(0x01.toByte()).lanewise(VectorOperators.LSHL, 4.toByte())
                            val fifthHi = qhVec.lanewise(VectorOperators.LSHR, bitHi)
                                .and(0x01.toByte()).lanewise(VectorOperators.LSHL, 4.toByte())
                            val codeLoBytes = loNib.lanewise(VectorOperators.OR, fifthLo)
                            val codeHiBytes = hiNib.lanewise(VectorOperators.OR, fifthHi)
                            val codeVecLo = codeLoBytes.castShape(floatSpecies, 0) as FloatVector
                            val codeVecHi = codeHiBytes.castShape(floatSpecies, 0) as FloatVector
                        ...
                        while (idx < SUB_BLOCK_SIZE) {
                            ...
                            val qh = weight[qhOffset + idx].toInt() and 0xFF
                            val codeLo = ((byte and 0x0F) or (((qh ushr bitLo.toInt()) and 0x01) shl 4)).toFloat()
                            val codeHi = ((byte ushr 4) or (((qh ushr bitHi.toInt()) and 0x01) shl 4)).toFloat()
    ...
    /**
     * IEEE 754 binary16 → binary32 conversion. Mirrors the helper in
     * [PanamaVectorQ4KMatmulKernel].
     */
