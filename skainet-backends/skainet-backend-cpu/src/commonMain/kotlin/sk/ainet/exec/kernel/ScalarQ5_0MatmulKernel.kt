...
/**
 * Scalar reference [Q5_0MatmulKernel] — per-block dequant + per-element FMA,
 * no SIMD. commonMain, so available on every KMP target.
 *
 * Block layout (32-elt, 22 B; block-major `(blockIdx*outputDim+o)*22`):
 * `d`(f16) `qh[4]` `qs[16]`. Dequant matches `DequantOps.dequantQ5_0FromBytes`:
 * `w = d*(code + (highBit shl 4) - 16)` (symmetric, no per-block min).
 */
public object ScalarQ5_0MatmulKernel : Q5_0MatmulKernel {
    ...
    override fun matmul(
        ...
        require(inputDim % BLOCK_SIZE == 0) {
            "ScalarQ5_0MatmulKernel: inputDim must be a multiple of $BLOCK_SIZE; got $inputDim"
        }
