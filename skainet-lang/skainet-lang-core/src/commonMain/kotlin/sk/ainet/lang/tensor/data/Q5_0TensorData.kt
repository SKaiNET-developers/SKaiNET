...
/**
 * Tensor data interface for the GGML **Q5_0** quantized format (5-bit, symmetric).
 *
 * Q5_0 block format (32 elements per block, 22 bytes per block):
 * - 2 bytes: f16 scale (`d`)
 * - 4 bytes: `qh[0..3]` — the 5th (high) bit of each of the 32 codes
 * - 16 bytes: `qs[0..15]` — the low 4 bits, two nibbles per byte
 *
 * Dequantization (matching `sk.ainet.io.gguf.dequant.DequantOps.dequantQ5_0FromBytes`):
 ...
 *
 * The `- 16` bias makes the 5-bit code symmetric around zero.
 */
public interface Q5_0TensorData : TensorData<DType, Byte> {
    /** Number of Q5_0 blocks in the tensor. */
    ...
    public companion object {
        ...
        /** Bytes per Q5_0 block (2 `d` + 4 `qh` + 16 `qs`). */
        public const val BYTES_PER_BLOCK: Int = 22
    }
}

/**
 * Implementation of [Q5_0TensorData] backed by a packed byte array, in the
 * natural GGUF **row-major** `[out, in]` layout. `matmulQ5_0Vec` indexes the
 * packed bytes row-major, so no block-major re-layout is needed.
 */
public class Q5_0BlockTensorData(
    ...
    init {
        val requiredBytes = blockCount * Q5_0TensorData.BYTES_PER_BLOCK
    ...
    override fun dequantizeBlock(blockIdx: Int, output: FloatArray, outputOffset: Int) {
        ...
        val qh0 = data[base + 2].toInt() and 0xFF
        val qh1 = data[base + 3].toInt() and 0xFF
        val qh2 = data[base + 4].toInt() and 0xFF
        val qh3 = data[base + 5].toInt() and 0xFF
        val qh = intArrayOf(qh0, qh1, qh2, qh3)
        val qsBase = base + 6
        val elemsInBlock = minOf(Q5_0TensorData.BLOCK_SIZE, shape.volume - blockIdx * Q5_0TensorData.BLOCK_SIZE)
        for (j in 0 until 16) {
            ...
            if (j < elemsInBlock && o0 < output.size) output[o0] = d * (lo + (bitLo shl 4) - 16)
            val o1 = outputOffset + 16 + j
            if (16 + j < elemsInBlock && o1 < output.size) output[o1] = d * (hi + (bitHi shl 4) - 16)
        }
    }
    ...
    override fun get(vararg indices: Int): Byte {
        ...
        dequantizeBlock(flatIndex / Q5_0TensorData.BLOCK_SIZE, tmp, 0)
        return tmp[flatIndex % Q5_0TensorData.BLOCK_SIZE].toInt().toByte()
    }

    override fun set(vararg indices: Int, value: Byte) {
        throw UnsupportedOperationException("Q5_0BlockTensorData is read-only (packed quantized weights)")
    }
    ...
    public companion object {
        /** Create [Q5_0BlockTensorData] from raw packed Q5_0 bytes (GGUF row-major). */
        public fun fromRawBytes(shape: Shape, bytes: ByteArray): Q5_0BlockTensorData =
            Q5_0BlockTensorData(shape, bytes)
    }
}

/** Dequantize Q5_0 tensor data to a FloatArray (row-major, matching the packed layout). */
public fun Q5_0TensorData.toFloatArray(): FloatArray {
    val result = FloatArray(shape.volume)
    val block = this as Q5_0BlockTensorData
    for (blockIdx in 0 until blockCount) {
        block.dequantizeBlock(blockIdx, result, blockIdx * Q5_0TensorData.BLOCK_SIZE)
    }
    return result
}
