...
class FashionMNISTIntegrationTest {
    ...
    fun downloadSplitAndTensorize() = runBlocking {
        ...
        assertEquals(FashionMNISTConstants.IMAGE_PIXELS, sample.image.size)

        // Verify label is in valid range and has a class name
        assertTrue(sample.label in 0..9, "Label should be between 0 and 9")
        assertTrue(sample.className() in FashionMNISTConstants.CLASS_NAMES, "Class name should be valid")

        // Convert to an Int8 tensor with shape [1, 28, 28]
        val ctx = DefaultDataExecutionContext()
        val shape = Shape(1, FashionMNISTConstants.IMAGE_SIZE, FashionMNISTConstants.IMAGE_SIZE)
        ...
        assertEquals(FashionMNISTConstants.IMAGE_PIXELS, tensor.volume)

        // Verify tensor isn't all zeros by comparing with original bytes
        // and also spot-check a few indices match exactly
        var nonZeroCount = 0
        val h = FashionMNISTConstants.IMAGE_SIZE
        val w = FashionMNISTConstants.IMAGE_SIZE
        for (yy in 0 until h) {
            for (xx in 0 until w) {
        ...
        // At least one pixel in Fashion-MNIST image should be non-zero
