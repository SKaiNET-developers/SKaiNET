...
To pin a package, add one line to `++[++versions++]++` in `gradle/libs.versions.toml` with an `npm-` prefix:
...
`sk.ainet.npm-pins` (applied on the root project) turns every such entry into a Yarn `resolutions` entry in *both* generated root `package.json` files, so one declaration covers the JS and Wasm dependency graphs. Then regenerate and commit the lockfiles:
...
Package names containing a dot (`socket.io`) or a scope (`++@++types/node`) cannot be spelled as a catalog alias; they need an entry in `npmPins ++{++ packageNameOverrides ++}++` in the root build script.
