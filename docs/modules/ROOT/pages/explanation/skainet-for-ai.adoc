...
For more complex initializations, use the `tensor` DSL inside a
`data { }` block — the idiomatic form. See
xref:how-to/build-tensors.adoc[How-to: Build Tensors] for the full
surface (single-tensor return, multi-tensor capture, named
`createDataMap`):
...
// Single tensor — the typed `data<T, V>(ctx)` overload returns the
// last expression in the block.
val myTensor = data<FP32, Float>(ctx) {
    tensor {
        shape(2, 2) {
...
// Multiple tensors — capture into outer `lateinit var`s.
lateinit var randomTensor: Tensor<FP32, Float>
lateinit var customInit:   Tensor<Int32, Int>
data(ctx) {
    randomTensor = tensor<FP32, Float> {
        shape(10, 10) { randn(mean = 0f, std = 1f) }
    }
    customInit = tensor<Int32, Int> {
        shape(5, 5) { init { indices -> indices[0] + indices[1] } }
    }
}
----
...
|*Creation* |`np.array(++[++1, 2, 3++]++)` |`data<FP32, Float>(ctx) ++{++ tensor ++{++ shape(3) ++{++ from(1f, 2f, 3f) } } }`
