...
class LlamaWeightMapperTest {
    ...
    fun `maps loader tensors into runtime weights with shape checks`() {
        ...
        val tensors = linkedMapOf(
            LlamaTensorNames.TOKEN_EMBEDDINGS to tensor(Shape(4, 8), 32, 0f),    // [dim, vocab]
            LlamaTensorNames.OUTPUT_NORM to tensor(Shape(4), 4, 100f),
            LlamaTensorNames.OUTPUT_WEIGHT to tensor(Shape(4, 8), 32, 200f),     // [dim, vocab]
            ...
            LlamaTensorNames.attnK(0) to tensor(Shape(4, 4), 16, 30f),           // [dim, kv_dim]
            LlamaTensorNames.attnV(0) to tensor(Shape(4, 4), 16, 40f),           // [dim, kv_dim]
            LlamaTensorNames.attnOut(0) to tensor(Shape(4, 4), 16, 50f),         // [dim, dim]
            LlamaTensorNames.ffnNorm(0) to tensor(Shape(4), 4, 60f),
            LlamaTensorNames.ffnGate(0) to tensor(Shape(4, 8), 32, 70f),         // [dim, ff_dim]
            LlamaTensorNames.ffnDown(0) to tensor(Shape(8, 4), 32, 80f),         // [ff_dim, dim]
            LlamaTensorNames.ffnUp(0) to tensor(Shape(4, 8), 32, 90f)            // [dim, ff_dim]
        )
        ...
        assertEquals(Shape(8, 4), layer.ffnDown.shape)
